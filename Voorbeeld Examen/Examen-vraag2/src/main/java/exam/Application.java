...
public class Application {
	...
	private static void removeDuplicates(List<Book> books) {
		...
		//Code
		HashSet<Book> booksSet = books.stream().collect(Collectors.toCollection(HashSet::new));
	...
	private static  void getBooksbefore(List<Book> books) {
		...
		//Code
		var oldBooks = books.stream().filter(x -> x.getReleaseYear() < 1950).collect(Collectors.toList());
